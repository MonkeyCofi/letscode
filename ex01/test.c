...
char	*replace(char *str, char *find_s, char *repl_s)
{
	...
	while (str[index])
	{
		...
		{
			repl_i = 0;
			while (is_found(&str[index], find_s, ft_strlen(&str[index])))
			{
				new[j++] = repl_s[repl_i++];
				if (!repl_s[repl_i])
				{
...
int main(void)
{
	...
	char *test4 = replace("to be or not to be. that is the question", "to", "hehe");
	printf("test1: %s\n", test1);
	printf("test2: %s\n", test2);
	printf("test3: %s\n", test3);
	printf("test4: %s\n", test4);
	...
	char *str = replace("bananabananabanana", "banana", "apple");
	printf("%s\n", str);
	free(str);
}